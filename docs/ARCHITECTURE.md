...
| `file-review.ts` | Tool schema (`fileReviewTool`, name `review_customer_files`) + `buildFileReviewContext({scope, request})`. Walks the customer's upload folder, extracts content per file type (image dimensions via header-parse, text/csv/md raw, xlsx via SheetJS, pdf via `pdf-parse`), bundles with the standards corpus slice for the requested scope, and returns a single string for the tool_result. Enforces `maxFilesPerReview` and `maxExtractCharsPerFile` caps. | `xlsx`, `pdf-parse`, `config.ts`, `customer-files.ts`, `node:fs`, `node:path` | `app/api/chat/route.ts` |
