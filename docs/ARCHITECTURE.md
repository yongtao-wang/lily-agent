...
**TL;DR.** Next.js 14 App Router. Two API routes (`/api/chat` for SSE streaming, `/api/upload` for files). Six `lib/` modules with clear single responsibilities. Server holds canonical session state in an in-memory `Map`; the client mirrors it for display. Every chat request rebuilds the system prompt from cached knowledge and a fresh stage/escalation snapshot.
...
Upload flow is the same shape but simpler: `app/api/upload/route.ts` parses multipart form data, validates mime + size against `config.upload`, writes files to `./uploads/{sessionId}/`, and appends the resulting `FileRef[]` to the session.
...
|---|---|---|---|
| `config.ts` | Single source of truth for tunables: model, customer info, stages, upload limits, escalation paths, placeholder design link, handoff message. Exports `config` object + `StageId` type + `getStageLabel(id)`. | — | all other lib files, `app/page.tsx`, both API routes |
...
| `prompt.ts` | Assembles the system prompt per request. Filters scripts by `stageId`. Emits the post-escalation directive when `escalated` is true. | `config.ts`, knowledge types | `app/api/chat/route.ts` |
| `claude.ts` | Anthropic SDK wrapper. `streamChat` handles the streaming + multi-iteration tool-use loop. Converts session messages to `MessageParam[]` and inlines image attachments as base64 image blocks. | `@anthropic-ai/sdk`, `config.ts`, session types, `node:fs`, `node:path` | `app/api/chat/route.ts` |
| `escalation.ts` | Tool schema (`escalationTool`) + `notifyProjectManager()` that writes the structured log block. | `config.ts`, session types, `node:fs`, `node:path` | `app/api/chat/route.ts` |
| `session.ts` | In-memory `Map<sessionId, Session>` with the CRUD helpers `getOrCreateSession`, `appendMessage`, `appendFiles`, `setStage`, `markEscalated`, `getRecentMessages`. Defines `ChatMessage`, `FileRef`, `Session` types. | — | both API routes, `claude.ts` (via type import), `escalation.ts` (via type import) |
...
| `api/chat/route.ts` | POST handler. Sets `runtime = 'nodejs'` and `dynamic = 'force-dynamic'`. Builds a `ReadableStream` that pumps SSE-formatted JSON events. Wires `onText` (→ `text` events) and `onToolUse` (→ runs escalation, emits `escalated` event). Appends user + final assistant messages to session. |
| `api/upload/route.ts` | POST handler using `request.formData()`. Validates `mime` against `config.upload.allowedMimeTypes` and size against `config.upload.maxSizeMB`. Returns 415/413 with Chinese error messages on rejection. Writes to `./uploads/{sessionId}/{timestamp}-{safeFilename}`. |
...
| `ComposerBar.tsx` | File picker (📎), textarea (Enter to send, Shift+Enter newline, respects IME composition), send button. Client-side validates files against `ALLOWED_MIME` and `MAX_SIZE_MB` before submitting; shows a red error banner on failure. |
...
Why we don't stream the tool_use block to the client: the spec (§9) wants the client to see only the natural language. The escalation event is signaled separately as `data: {"type":"escalated"}` so the UI could show a badge if it wanted.
...
| No auth, customer hardcoded | URL token / OAuth — touch `page.tsx` and `config.ts` |
...
- **`@anthropic-ai/sdk@0.32.1`** predates `DocumentBlockParam`. PDFs uploaded by the customer are passed to Claude as text markers like `[客户上传文件: report.pdf (application/pdf, 240 KB)]` (see `attachmentsToBlocks` in `lib/claude.ts`). Spec says "PDFs stored, not parsed" so this is acceptable. Bumping the SDK lets you pass PDFs as `{type: 'document', source: {type: 'base64', ...}}` — one block in `lib/claude.ts` to change.
...
- [ESCALATION.md](ESCALATION.md) — tool schema, log format, swap path
